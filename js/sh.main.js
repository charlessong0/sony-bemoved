...
!function (SH) {
    !function (Main) {
        ...
        {
            var DEFAULT_ASSET_VERSION = 'FINAL_W_MOBILE', ASSET_VERSION = (location.search.match(/a(?:sset_version)?=([^&]+)/) || [])[1] || DEFAULT_ASSET_VERSION, DEFAULT_DEEPLINK = null, QUERYSTRING_DEEPLINK = (location.search.match(/p?=([^&]+)/) || [])[1] || DEFAULT_DEEPLINK, hash = location.hash.replace('#', '') || QUERYSTRING_DEEPLINK;
        ...
        $('.viewport-inner').on('click', '.video-link', function (e) {
            e.preventDefault();
            var isMobile = SH.Config.isMobile, openDirectLinkInMobile = (-1 != navigator.userAgent.toLowerCase().indexOf('android'), !1);
